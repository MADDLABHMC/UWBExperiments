...
/*
if (dac.begin(0x60)) {
    Serial.println("Addy is 60");
    
  }
  else{
    Serial.print("Addy is not 60!");
  }

  if (dac.begin(0x62)){
      Serial.print("Addy is 62");
     
    }
  else{
    Serial.print("Addy is not 62");
   

   Result: Addy is not 62Addy is 60
Addy is not 62Addy is 60
...
/*const PROGMEM uint16_t DACLookup_FullSine_8Bit[128] =
{
...
// 6 bit sin wave with only 64 values allows us to shove out these values alot faster, hence haster update and oscillation 
...
void loop() {
  // put your main code here, to run repeatedly
        ...
        if(CurrentTime - PreviousTime >= TimeDelay){
            dac.setVoltage(pgm_read_word(&(DACLookup_FullSine_6Bit[i])), false); // this pgm_read_word is necessary, as you are looking into the program memory that the esp32 specifically stored to check for the memory
            //address of where the memory address of the DACLookup chart is, and the false key word is there to make sure we don't store this last value inside of the MCP, as that way we would be wasting our memory. THere is a limit to how many times we can store before the part just breaks. 
       ...
       //regular Serial.print() was causing unknown characters in bits that I'm assuming we just couldn't handle.
      
      // Generate sin wave with all these value with a freq below
